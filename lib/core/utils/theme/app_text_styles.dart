import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:quiz/core/utils/theme/app_theme.dart';

class AppTextStyles {
  static TextStyle font26BoldBlack = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 26.sp,
    color: Colors.black,
  );
  static TextStyle font70BoldBlack = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 70.sp,
    color: Colors.black,
  );

  static TextStyle font16BlackBold = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 16.sp,
    color: Colors.black87,
  );

  static TextStyle font16Black = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 16.sp,
    color: Colors.black87,
  );

  static TextStyle font56BoldBlack = TextStyle(
    fontSize: 56.sp,
    fontWeight: FontWeight.bold,
    color: Colors.black,
    height: 1.2,
  );

  static TextStyle font20Grey = TextStyle(
    fontSize: 20.sp,
    color: Colors.grey.shade700,
    height: 1.4,
  );

  static TextStyle font35Grey = TextStyle(
    fontSize: 35.sp,
    color: Colors.grey.shade700,
    height: 1.4,
  );
  static TextStyle font22BlueSemiBold = TextStyle(
    fontSize: 22.sp,
    color: AppColors.blue700,
    fontWeight: FontWeight.w700,
  );

  static TextStyle font22White = TextStyle(
    fontSize: 22.sp,
    fontWeight: FontWeight.bold,
    color: Colors.white,
  );

  static TextStyle font35White = TextStyle(
    fontSize: 35.sp,
    fontWeight: FontWeight.bold,
    color: Colors.white,
  );

  static TextStyle font14WhiteBold = TextStyle(
    fontSize: 14.sp,
    fontWeight: FontWeight.bold,
    color: Colors.white,
  );
  static TextStyle font30BoldBlack = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 30.sp,
    color: Colors.black87,
  );

  static TextStyle font45BoldBlack = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 45.sp,
    color: Colors.black87,
  );

  static TextStyle font45BoldBlue = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 45.sp,
    color: Colors.blue,
  );

  static TextStyle font45BoldWhite = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 45.sp,
    color: Colors.white,
  );

  static TextStyle font18RegularGrey = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 18.sp,
    color: Colors.grey.shade700,
  );

  static TextStyle font20BoldBlue300 = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 20.sp,
    color: Colors.blue.shade300,
  );

  static TextStyle font17SemiBoldGrey = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 17.sp,
    color: Colors.grey.shade400,
    height: 1.4,
  );

  static TextStyle font14Black = TextStyle(
    fontSize: 14.sp,
    color: Colors.black,
  );

  static TextStyle font14LightBlue = TextStyle(
    fontSize: 14.sp,
    color: Colors.blue,
  );

  static TextStyle font14White = TextStyle(
    fontSize: 14.sp,
    color: Colors.white,
  );

  static TextStyle font14BlackBold = TextStyle(
    fontSize: 14.sp,
    color: Colors.black,
    fontWeight: FontWeight.w600,
  );

  static TextStyle font14Blue = TextStyle(
    fontSize: 14.sp,
    color: Color(0xFF0066CC),
  );

  static TextStyle font16BoldBlue = TextStyle(
    fontSize: 16.sp,
    fontWeight: FontWeight.bold,
    color: const Color(0xFF0066CC),
  );

  static TextStyle font24BoldBlack = TextStyle(
    fontSize: 24.sp,
    fontWeight: FontWeight.bold,
    color: Colors.black,
  );
  static TextStyle font14Grey = TextStyle(
    fontSize: 14.sp,
    color: Colors.grey[600],
  );

  static TextStyle font12BoldGrey = TextStyle(
    fontSize: 12.sp,
    color: Colors.grey[600],
  );

  static TextStyle font12Regular = TextStyle(
    fontSize: 12.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle font17Grey = TextStyle(
    fontSize: 17.sp,
    color: Colors.grey[500],
  );

  static TextStyle font20SemiBoldBlack = TextStyle(
    fontSize: 20.sp,
    color: Colors.black87,
    fontWeight: FontWeight.w600,
  );

  static TextStyle font15SemiBoldBlack = TextStyle(
    fontSize: 15.sp,
    color: Colors.black87,
    fontWeight: FontWeight.w600,
  );

  static TextStyle font50SemiBoldBlack = TextStyle(
    fontSize: 50.sp,
    color: Colors.black87,
    fontWeight: FontWeight.w600,
  );

  static TextStyle font12White70 = TextStyle(
    fontSize: 12.sp,
    color: Colors.white.withOpacity(0.7),
  );

  static TextStyle font16BoldWhite = TextStyle(
    fontSize: 16.sp,
    color: Colors.white,
    fontWeight: FontWeight.bold,
  );

  static TextStyle font16Grey = TextStyle(
    fontWeight: FontWeight.w500,
    fontSize: 16.sp,
    color: Colors.grey.shade300,
    height: 1.4,
  );

  static TextStyle font18BoldBlue300 = TextStyle(
    fontWeight: FontWeight.bold,
    fontSize: 18.sp,
    color: Colors.blue.shade300,
  );

  static TextStyle font18Grey300 = TextStyle(
    fontWeight: FontWeight.w400,
    color: Colors.grey.shade400,
    fontSize: 18.sp,
    height: 1.3,
  );

  static TextStyle font14BoldBlack = TextStyle(
    fontSize: 14.sp,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle font16BoldBlack = TextStyle(
    fontSize: 16.sp,
    color: Colors.black,
    fontWeight: FontWeight.bold,
  );

  static TextStyle font12Grey = TextStyle(fontSize: 12.sp, color: Colors.grey);
  static TextStyle font40Grey = TextStyle(fontSize: 40.sp, color: Colors.grey);

  static TextStyle font16White = TextStyle(
    fontSize: 16.sp,
    color: Colors.white,
  );

  static TextStyle font20BoldWhite = TextStyle(
    fontSize: 20.sp,
    color: Colors.white,
    fontWeight: FontWeight.bold,
  );
}
